...
function Tree:new(midx)
  local tree
  if(trees.numChildren > 1) then
    --set a width of 20,40,60,80,or 100, randomly
    local width = math.random(1,5)*20  
  	tree = display.newImage("graphics/tree_" .. width .. ".png")
    tree.width = width
  else
  	tree = display.newImage("graphics/tree_100.png")
    tree.width = 100
  end
  tree.midx = midx  
  ...
  print("Tree:new, width = " .. tree.width .. ", midx = " .. (midx or "nil"))  
  ...
  function tree:destroy()
    ...
    local burningTree = display.newImage("graphics/tree_" .. self.width .. "_destroyed.png")
